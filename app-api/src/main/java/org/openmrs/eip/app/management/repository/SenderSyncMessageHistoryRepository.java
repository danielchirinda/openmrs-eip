...
public interface SenderSyncMessageHistoryRepository extends JpaRepository<SenderSyncMessageDetail, Long> {
	
	@Query(value = " select table_.table_name as tableName, received as receivedItens, sent as sentItens, newer as  nonReceivedItens "
			+ " from "
			+ "	( "
			+ "		(select distinct(table_name) table_name  from sender_sync_message_detail) table_ "
			+ "			left join (select table_name, count(*) received  "
			+ "						from sender_sync_message_detail  "
